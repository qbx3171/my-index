...
C.injectGlobalStyle=function(){
  ...
  s.id='communityGlobalStyle';
  s.textContent=
    '#communityNotifPanel *{scrollbar-width:thin;scrollbar-color:transparent transparent;}'+
    '#communityNotifPanel *::-webkit-scrollbar{width:6px;height:6px;background:transparent;}'+
    '#communityNotifPanel *::-webkit-scrollbar-track{background:transparent;}'+
    '#communityNotifPanel *::-webkit-scrollbar-thumb{background:transparent;border-radius:6px;}'+
    '#communityNotifPanel *::-webkit-scrollbar-thumb:hover{background:rgba(0,119,255,0.18);}'+
    '#communityNotifDetail *{scrollbar-width:thin;scrollbar-color:transparent transparent;}'+
    '#communityNotifDetail *::-webkit-scrollbar{width:6px;height:6px;background:transparent;}'+
    '#communityNotifDetail *::-webkit-scrollbar-track{background:transparent;}'+
...
C.init=function(){
  C.injectGlobalStyle();
  if(typeof supabaseClient==='undefined'||!supabaseClient){setTimeout(C.init,500);return;}
  C.run();
  C.retry();
};
...
C.retry=function(){
  ...
  C._rt=setInterval(function(){
    ...
    try{
      ...
      if(pp&&!pp.classList.contains('hidden'))C.injectProfile();
    }catch(e){}
  },1000);
};
...
C.bindAuth=function(){
  ...
  C._ab=true;
  try{
    supabaseClient.auth.onAuthStateChange(function(e,s){
      if(s&&s.user){window.currentUser=s.user;C.onLogin();}
      else{window.currentUser=null;C.onLogout();}
    });
  }catch(e){}
  setTimeout(function(){if(window.currentUser)C.onLogin();},1500);
};

C.onLogin=async function(){
  C.refreshProfile();
  try{await C.ensureProfile();}catch(e){}
...
C.ensureProfile=async function(){
  if(!window.currentUser)return;
  var r=await supabaseClient.from('profiles').select('id').eq('id',window.currentUser.id).maybeSingle();
  if(r.error||r.data)return;
  await supabaseClient.from('profiles').insert({id:window.currentUser.id,points:0,level:1,streak:0,updated_at:new Date().toISOString()});
};

C.syncFav=async function(){
  if(!window.currentUser)return;
  var r=await supabaseClient.from('favorites').select('software_id').eq('user_id',window.currentUser.id);
  if(r.error)return;
  var ids=(r.data||[]).map(function(x){return x.software_id;});
  localStorage.setItem('lezhe_favorites',JSON.stringify(ids));
};

C.syncHist=async function(){
  if(!window.currentUser)return;
  var r=await supabaseClient.from('view_history').select('software_id').eq('user_id',window.currentUser.id).order('viewed_at',{ascending:false}).limit(6);
  if(r.error)return;
  var ids=(r.data||[]).map(function(x){return x.software_id;});
...
C.addFavCloud=async function(id){
  if(!window.currentUser)return;
  await supabaseClient.from('favorites').upsert({user_id:window.currentUser.id,software_id:String(id)},{onConflict:'user_id,software_id'});
};

C.rmFavCloud=async function(id){
  if(!window.currentUser)return;
  await supabaseClient.from('favorites').delete().eq('user_id',window.currentUser.id).eq('software_id',String(id));
};

C.addHistCloud=async function(id){
  if(!window.currentUser)return;
  await supabaseClient.from('view_history').upsert({user_id:window.currentUser.id,software_id:String(id),viewed_at:new Date().toISOString()},{onConflict:'user_id,software_id'});
};

C.loadPoints=async function(){
  if(!window.currentUser)return;
  var r=await supabaseClient.from('profiles').select('points,level,streak,last_checkin').eq('id',window.currentUser.id).maybeSingle();
  if(r.error)return;
  if(r.data){
    window.userPoints=r.data.points||0;
    window.userLevel=r.data.level||1;
...
C.addPoints=async function(p){
  if(!window.currentUser)return;
  var pr=await supabaseClient.from('profiles').select('points').eq('id',window.currentUser.id).maybeSingle();
  var cur=(pr.data&&pr.data.points)||0;
  var np=cur+p;
  var lv=Math.floor(np/100)+1;
  var r=await supabaseClient.from('profiles').upsert({id:window.currentUser.id,points:np,level:lv,updated_at:new Date().toISOString()},{onConflict:'id'});
  if(r.error){C.tip('积分保存失败','error');return;}
  await C.loadPoints();
  C.updateLv();
  C.refreshProfile();
  C.tip('积分 +'+p,'success');
...
C.updateLv=function(){
  ...
  var b=document.getElementById('communityLevelBadge');
  if(!b){
    var an=document.getElementById('authNav');
    if(!an)return;
    b=document.createElement('span');
    b.id='communityLevelBadge';
    b.style.cssText='font-size:0.6rem;background:var(--accent-gradient);color:#fff;padding:1px 6px;border-radius:20px;margin-left:4px;line-height:1.4;';
    an.appendChild(b);
  }
  b.textContent='Lv'+lv;
};
...
C.showNotifs=function(){
  ...
  p.style.cssText='position:fixed;top:70px;right:16px;width:380px;max-width:calc(100vw - 32px);max-height:520px;background:var(--bg-card-solid);border:1px solid var(--border-glow-strong);border-radius:18px;box-shadow:0 20px 56px rgba(0,0,0,0.18);z-index:9999;overflow:hidden;display:flex;flex-direction:column;animation:notifIn .22s cubic-bezier(0.2,0,0,1);';
  var styleEl=document.createElement('style');
  styleEl.textContent='@keyframes notifIn{from{opacity:0;transform:translateY(-8px) scale(0.98);}to{opacity:1;transform:translateY(0) scale(1);}}';
  ...
  var bd=document.createElement('div');
  bd.id='communityNotifBody';
  bd.style.cssText='flex:1;overflow-y:auto;padding:10px;background:var(--bg-primary);min-height:120px;';
  bd.innerHTML='<div style="text-align:center;padding:40px 20px;color:var(--text-dim);font-size:0.8rem;"><i class="fas fa-spinner fa-spin"></i> 加载中...</div>';
  p.appendChild(bd);
...
C.loadNotifContent=async function(bd){
  try{
    ...
    if(cntEl)cntEl.innerHTML=(ur>0?('<span style="color:#0077ff;font-weight:600;">'+ur+'</span> 条未读'):'全部已读');
...
C.hijackDetail=function(){
  if(!window.Detail||Detail._ch)return;
  if(typeof Detail.open!=='function')return;
  Detail._ch=true;
  var o=Detail.open;
  Detail.open=function(s){o.call(Detail,s);C.currentSoft=s;C.addHistCloud(s.id);C.injectDetail(s);};
};

C.hijackProfile=function(){
  if(!window.App||App._ch)return;
...
C.hijackFav=function(){
  if(typeof toggleFavorite!=='function'||toggleFavorite._ch)return;
  toggleFavorite._ch=true;
  var o=toggleFavorite;
  window.toggleFavorite=function(id){var r=o(id);if(r)C.addFavCloud(id);else C.rmFavCloud(id);return r;};
};

C.hijackHist=function(){
  if(typeof addViewHistory!=='function'||addViewHistory._ch)return;
  addViewHistory._ch=true;
  var o=addViewHistory;
...
C.injectDetail=function(s){
  ...
  sec.id='communityDetailSection';
  sec.style.cssText='margin-top:16px;border-top:1px solid var(--border-glow);padding-top:12px;';
  sec.innerHTML='<h4 style="font-size:0.9rem;margin-bottom:8px;"><i class="fas fa-star"></i> 评分与评论</h4><div id="communityRatingBox"></div><div id="communityReviewList"></div><div style="margin-top:10px;"><textarea id="communityReviewContent" placeholder="写下你的评价..." style="width:100%;padding:8px;border:1px solid var(--border-glow);border-radius:8px;background:var(--bg-primary);color:var(--text-primary);font-size:0.8rem;min-height:60px;"></textarea><div style="display:flex;gap:8px;margin-top:6px;align-items:center;"><select id="communityRatingSelect" style="padding:4px 8px;border-radius:6px;border:1px solid var(--border-glow);background:var(--bg-primary);color:var(--text-primary);font-size:0.8rem;"><option value="5">5星</option><option value="4">4星</option><option value="3">3星</option><option value="2">2星</option><option value="1">1星</option></select><button class="btn btn-sm btn-primary" id="communitySubmitReview">发表评价</button></div></div><h4 style="font-size:0.9rem;margin:16px 0 8px;"><i class="fas fa-comments"></i> 讨论区</h4><div id="communityDiscussionList"></div><div style="margin-top:10px;"><textarea id="communityDiscussionContent" placeholder="参与讨论..." style="width:100%;padding:8px;border:1px solid var(--border-glow);border-radius:8px;background:var(--bg-primary);color:var(--text-primary);font-size:0.8rem;min-height:60px;"></textarea><button class="btn btn-sm btn-primary" id="communitySubmitDiscussion" style="margin-top:6px;">发表讨论</button></div>';
  mb.appendChild(sec);
  C.loadReviews(s.id);
  C.loadDiscussions(s.id);
  document.getElementById('communitySubmitReview').onclick=function(){C.submitReview(s.id);};
  document.getElementById('communitySubmitDiscussion').onclick=function(){C.submitDiscussion(s.id);};
};

C.loadReviews=async function(id){
  var list=document.getElementById('communityReviewList');
  if(!list)return;
  var r=await supabaseClient.from('software_reviews').select('*').eq('software_id',String(id)).order('created_at',{ascending:false});
  if(r.error){list.innerHTML='<p style="color:var(--text-dim);font-size:0.8rem;">暂无评价</p>';return;}
  var d=r.data||[];
  if(d.length===0){list.innerHTML='<p style="color:var(--text-dim);font-size:0.8rem;">暂无评价，来写第一条吧</p>';return;}
  var avg=0,h='';
  for(var i=0;i<d.length;i++){
    var x=d[i];
    avg+=x.rating;
    var st='';
    for(var j=1;j<=5;j++){st+=j<=x.rating?'★':'☆';}
    h+='<div style="padding:8px 0;border-bottom:1px solid var(--border-glow);"><div style="display:flex;justify-content:space-between;font-size:0.75rem;"><span style="color:var(--accent-cyan);">'+st+'</span><span style="color:var(--text-dim);">'+(typeof timeAgo==='function'?timeAgo(x.created_at):'')+'</span></div><div style="font-size:0.8rem;margin-top:2px;">'+C.esc(x.content||'')+'</div></div>';
  }
  avg=(avg/d.length).toFixed(1);
  var rb=document.getElementById('communityRatingBox');
  if(rb)rb.innerHTML='<div style="font-size:1.2rem;font-weight:700;color:var(--accent-cyan);">'+avg+' 分</div><div style="font-size:0.7rem;color:var(--text-dim);">共 '+d.length+' 条评价</div>';
  list.innerHTML=h;
};

C.submitReview=async function(id){
  if(!window.currentUser){C.needLogin();return;}
  var ce=document.getElementById('communityReviewContent');
  var re=document.getElementById('communityRatingSelect');
  if(!ce||!re)return;
  var c=ce.value.trim();
  var r=parseInt(re.value);
  if(!c){C.tip('请填写评价内容','warning');return;}
  var res=await supabaseClient.from('software_reviews').upsert({user_id:window.currentUser.id,software_id:String(id),rating:r,content:c},{onConflict:'user_id,software_id'});
  if(res.error){C.tip('评价失败: '+res.error.message,'error');return;}
  C.tip('评价成功','success');
  ce.value='';
  C.loadReviews(id);
  await C.addPoints(5);
...
C.loadDiscussions=async function(id){
  var list=document.getElementById('communityDiscussionList');
  if(!list)return;
  var r=await supabaseClient.from('software_discussions').select('*').eq('software_id',String(id)).is('parent_id',null).order('created_at',{ascending:false});
  if(r.error){list.innerHTML='<p style="color:var(--text-dim);font-size:0.8rem;">暂无讨论</p>';return;}
  var d=r.data||[];
  if(d.length===0){list.innerHTML='<p style="color:var(--text-dim);font-size:0.8rem;">暂无讨论，来发表第一条吧</p>';return;}
  var h='';
  for(var i=0;i<d.length;i++){
    var x=d[i];
    h+='<div style="padding:8px 0;border-bottom:1px solid var(--border-glow);"><div style="font-size:0.7rem;color:var(--text-dim);">'+(typeof timeAgo==='function'?timeAgo(x.created_at):'')+'</div><div style="font-size:0.8rem;margin-top:2px;">'+C.esc(x.content)+'</div></div>';
  }
  list.innerHTML=h;
};

C.submitDiscussion=async function(id){
  if(!window.currentUser){C.needLogin();return;}
  var ce=document.getElementById('communityDiscussionContent');
  if(!ce)return;
  var c=ce.value.trim();
  if(!c){C.tip('请填写讨论内容','warning');return;}
  var r=await supabaseClient.from('software_discussions').insert({user_id:window.currentUser.id,software_id:String(id),content:c});
  if(r.error){C.tip('发表失败','error');return;}
  C.tip('发表成功','success');
  ce.value='';
  C.loadDiscussions(id);
  await C.addPoints(3);
};
...
C.doCheckin=async function(){
  if(!window.currentUser){C.tip('请先登录','warning');return;}
  var b=document.getElementById('communityCheckinBtn');
  if(b){b.disabled=true;b.textContent='签到中...';}
  try{
    var td=C.getToday();
    var ex=await supabaseClient.from('checkins').select('id').eq('user_id',window.currentUser.id).eq('checkin_date',td).maybeSingle();
    if(ex.error){C.tip('查询失败','error');if(b){b.disabled=false;b.textContent='每日签到';}return;}
    if(ex.data){
      C.tip('今天已经签到过了','warning');
      window.userLastCheckin=td;
      await C.loadPoints();
      C.injectProfile();
      C.updateLv();
      return;
    }
    var pr=await supabaseClient.from('profiles').select('points,streak,last_checkin').eq('id',window.currentUser.id).maybeSingle();
    var cp=0,cs=0,lc=null;
    if(pr.data){cp=pr.data.points||0;cs=pr.data.streak||0;lc=pr.data.last_checkin||null;}
    var st=cs+1;
    if(st>1&&lc){var df=(new Date(td)-new Date(lc))/86400000;if(df>1)st=1;}
    var np=cp+10;
    var lv=Math.floor(np/100)+1;
    var ir=await supabaseClient.from('checkins').insert({user_id:window.currentUser.id,checkin_date:td,points:10});
    if(ir.error){C.tip('签到失败','error');if(b){b.disabled=false;b.textContent='每日签到';}return;}
    var ur=await supabaseClient.from('profiles').upsert({id:window.currentUser.id,points:np,level:lv,streak:st,last_checkin:td,updated_at:new Date().toISOString()},{onConflict:'id'});
    if(ur.error){C.tip('积分保存失败','error');if(b){b.disabled=false;b.textContent='每日签到';}return;}
    await C.loadPoints();
    C.injectProfile();
    C.updateLv();
    C.tip('签到成功 +10 积分','success');
  }catch(e){
    C.tip('签到异常','error');
...
C.loadBoard=async function(){
  ...
  if(!el)return;
  el.innerHTML='加载中...';
  var r=await supabaseClient.from('profiles').select('username,points,level').order('points',{ascending:false}).limit(10);
  if(r.error){el.innerHTML='<p style="color:var(--text-dim);">加载失败</p>';return;}
  var d=r.data||[];
  if(d.length===0){el.innerHTML='<p style="color:var(--text-dim);">暂无数据</p>';return;}
  var h='';
  for(var i=0;i<d.length;i++){
    var u=d[i];
...
C.egg=function(){
  var lg=document.getElementById('logoHome');
  if(lg&&!lg._ee){
    lg._ee=true;
    var n=0;
    lg.addEventListener('click',function(){
      ...
      if(n>=5){
        ...
        r.style.setProperty('--accent-cyan','#ff00ff');
        setTimeout(function(){r.style.setProperty('--accent-cyan','#0077ff');},3000);
      }
    });
  }
};
